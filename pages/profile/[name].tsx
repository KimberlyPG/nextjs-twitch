...
const Profile: NextPage<ProfileProps> = ({ userData }) => {
    ...
    const { data: video, error, isLoading } = useSWR<Video[]>(`/videos?user_id=${userId}&first=5`);
            ...
            {state === "true" &&
                <div className='pl-3'>
                    <Link href={`/stream/${userData?.display_name}`}>
                        <p className='bg-red-600 text-center rounded-sm font-bold font-roboto p-1 hover:opacity-50 w-28 xs:text-sm lg:text-lg'>
                            GO TO LIVE
                        </p>
                    </Link>
                </div>
